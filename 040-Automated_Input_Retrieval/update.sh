...
function process_eia_petroleum() {
    ...
    cd "${DESTDIR}"
    wget --no-clobber -r -l1 -q --progress=bar --no-parent -A "*.xls,*.xlsx" "${URL}"
    cd "${BASEDIR}"
...
function process_eia_seds() {
    ...
    wget -L "${URL_SEDS_CSV}" -O "${FILE_SEDS_CSV}"
    wget -L "${URL_SEDS_CODES}" -O "${FILE_SEDS_CODES}"
